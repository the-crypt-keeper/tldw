...
function setupEventListeners() {
  ...
  document.getElementById('processAllTabs').addEventListener('click', processAllTabs);
  document.getElementById('saveAllBookmarks').addEventListener('click', saveAllBookmarks);
  document.getElementById('processSelectedTabs').addEventListener('click', processSelectedTabs);
  ...
  async performSearch(type, query) {
    try {
      ...
      if (this.searchCache.has(cacheKey)) {
        ...
        this.displayResults(type, results, query);
        return;
      }
      ...
      if (type === 'prompts') {
        if (query) {
          const response = await apiClient.searchPrompts(query, filters);
        ...
        if (query) {
          const response = await apiClient.searchCharacters(query, filters);
          results = response.results;
        } else {

...
test_python_version() {
    ...
    $python_version -m venv venv_$python_version
    ...
    pip install -e ".[dev]" 2>&1 | tee install_$python_version.log
    print_result ${PIPESTATUS[0]} "Package installation"
    ...
    python -c "import black; print(f'✓ black: {black.__version__}')" || echo "✗ black not found"
    python -c "import ruff; print(f'✓ ruff installed')" || echo "✗ ruff not found"
    python -c "import mypy; print(f'✓ mypy: {mypy.__version__}')" || echo "✗ mypy not found"
    
    # Test 2: Check pytest markers
    echo -e "\n${YELLOW}Test 2: Checking pytest markers${NC}"
    cd tldw_Server_API
    pytest --markers | grep -E "unit|integration|external_api" && print_result 0 "Pytest markers found" || print_result 1 "Pytest markers"
    
    # Test 3: Run unit tests (dry run to check discovery)
    echo -e "\n${YELLOW}Test 3: Test discovery with markers${NC}"
    pytest --collect-only -m "unit" 2>&1 | tee test_discovery_$python_version.log
    test_count=$(pytest --collect-only -q -m "unit" 2>/dev/null | grep -c "test" || echo "0")
    echo "Found $test_count unit tests"
    [ $test_count -gt 0 ] && print_result 0 "Unit test discovery" || print_result 1 "Unit test discovery"
    ...
    [ $black_result -eq 0 ] || [ $black_result -eq 1 ] && print_result 0 "Black formatter check" || print_result 1 "Black formatter check"
    
    # Ruff check
    echo "Running ruff check..."
    ruff check . 2>&1 | head -20
    ruff_result=${PIPESTATUS[0]}
    [ $ruff_result -eq 0 ] || [ $ruff_result -eq 1 ] && print_result 0 "Ruff linter check" || print_result 1 "Ruff linter check"
    
    # MyPy check (allow failures)
    echo "Running mypy check..."
    mypy . 2>&1 | head -20 || true
    print_result 0 "MyPy type checker (allowed to fail)"
    
    # Test 5: Coverage tools
    echo -e "\n${YELLOW}Test 5: Coverage tools${NC}"
    python -c "import pytest_cov; print('✓ pytest-cov installed')" && print_result 0 "Coverage tools" || print_result 1 "Coverage tools"
...
SERVER_PID=$!
sleep 5
curl -f http://localhost:8000/docs > /dev/null 2>&1 && print_result 0 "Server startup" || print_result 1 "Server startup"
...
# Summary
echo -e "\n${YELLOW}Summary:${NC}"
echo "- Tested dependency installation with pyproject.toml"
echo "- Verified pytest markers and test discovery"
echo "- Checked code quality tools (black, ruff, mypy)"
echo "- Tested coverage tools installation"
echo "- Validated server startup"

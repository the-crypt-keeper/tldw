...
def create_summarize_explain_tab():
    with gr.TabItem("Explain/Summarize Text"):
        ...
        explain_summarize_button.click(
            # FIXME - update checks for checkboxes and prompts
...
def summarize_explain_text(message, api_endpoint, api_key, user_prompt, system_prompt=None, temp=0.7, summarization=True, explanation=False):
    try:
        ...
        logging.info(f"Debug - summarize_explain_text Function - Prompt: {system_prompt}")
        logging.info(f"Debug - summarize_explain_text Function - Prompt: {user_prompt}")
        ...
        if explanation:
            ...
            if api_endpoint.lower() == 'openai':
                ...
                raise ValueError(f"Unsupported API endpoint: {api_endpoint}")
        if summarization_response is not None:

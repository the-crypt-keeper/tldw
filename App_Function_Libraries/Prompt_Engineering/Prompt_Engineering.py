...
# Function for generating the prompt based on the metaprompt


# Define function to generate the prompt based on user inputs
def generate_prompt(api_endpoint, api_key, task, variables_str, temperature):
    ...
    # Construct the metaprompt (adjust as needed from your metaprompt definition)
    metaprompt = f'''
    Today you will be writing instructions for an AI assistant that needs careful instruction.
    Here is the task for which you will write instructions: {task}.
    Here are the variables: {', '.join(variables)}.
    '''

    # Use chat API call to generate the prompt
    response = chat_api_call(api_endpoint=api_endpoint, api_key=api_key, input_data="",
                             prompt=metaprompt, temp=temperature)
    return response
...
def test_generated_prompt(api_endpoint, api_key, generated_prompt, variable_values_str, temperature):
    ...
    # Send the prompt with actual values to the chat API
    response = chat_api_call(api_endpoint=api_endpoint, api_key=api_key, input_data="",
                             prompt=prompt_with_values, temp=temperature)
    return response
